...
def show_table_data(table_name):
    ...
    for row in table_data:
        ...
        print(row_str)
    print("Query executed at:", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
cursor.execute("SELECT trip_name, location, SUM(total_completed_trips) AS total_completed_trips FROM trip t JOIN completed_trips ct ON t.trip_id = ct.trip_id GROUP BY t.trip_id ORDER BY total_completed_trips LIMIT 3")
result = cursor.fetchall()
print("Query executed at:", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
